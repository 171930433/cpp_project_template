...
namespace {
#ifdef USES_WIN32
std::unordered_map<std::string, unsigned int> KeySymToVKeyCode = { { "Cancel", 3 }, { "BackSpace", 8 }, { "Tab", 9 },
  { "Clear", 12 }, { "Return", 13 }, { "Shift_L", 16 }, { "Control_L", 17 }, { "Alt_L", 18 }, { "Pause", 19 },
  { "Caps_Lock", 20 }, { "Escape", 27 }, { "space", 32 }, { "Prior", 33 }, { "Next", 34 }, { "End", 35 },
  { "Home", 36 }, { "Left", 37 }, { "Up", 38 }, { "Right", 39 }, { "Down", 40 }, { "Select", 41 }, { "Execute", 43 },
  { "Snapshot", 44 }, { "Insert", 45 }, { "Delete", 46 }, { "Help", 47 }, { "1", 49 }, { "2", 50 }, { "3", 51 },
  { "4", 52 }, { "5", 53 }, { "6", 54 }, { "7", 55 }, { "8", 56 }, { "9", 57 }, { "a", 65 }, { "b", 66 }, { "c", 67 },
  { "d", 68 }, { "e", 69 }, { "f", 70 }, { "g", 71 }, { "h", 72 }, { "i", 73 }, { "j", 74 }, { "k", 75 }, { "l", 76 },
  { "m", 77 }, { "n", 78 }, { "o", 79 }, { "p", 80 }, { "q", 81 }, { "r", 82 }, { "s", 83 }, { "t", 84 }, { "u", 85 },
  { "v", 86 }, { "w", 87 }, { "x", 88 }, { "y", 89 }, { "z", 90 }, { "Win_L", 91 }, { "Win_R", 92 }, { "App", 93 },
  { "KP_0", 96 }, { "KP_1", 97 }, { "KP_2", 98 }, { "KP_3", 99 }, { "KP_4", 100 }, { "KP_5", 101 }, { "KP_6", 102 },
  { "KP_7", 103 }, { "KP_8", 104 }, { "KP_9", 105 }, { "asterisk", 106 }, { "plus", 107 }, { "bar", 108 },
  { "minus", 109 }, { "period", 110 }, { "slash", 111 }, { "F1", 112 }, { "F2", 113 }, { "F3", 114 }, { "F4", 115 },
  { "F5", 116 }, { "F6", 117 }, { "F7", 118 }, { "F8", 119 }, { "F9", 120 }, { "F10", 121 }, { "F11", 122 },
  { "F12", 123 }, { "F13", 124 }, { "F14", 125 }, { "F15", 126 }, { "F16", 127 }, { "F17", 128 }, { "F18", 129 },
  { "F19", 130 }, { "F20", 131 }, { "F21", 132 }, { "F22", 133 }, { "F23", 134 }, { "F24", 135 }, { "Num_Lock", 144 },
  { "Scroll_Lock", 145 } };
#endif
...
const std::unordered_map<int, int> imguiToVtkCursors(
  { { ImGuiMouseCursor_None, VTK_CURSOR_DEFAULT }, { ImGuiMouseCursor_Arrow, VTK_CURSOR_ARROW },
    { ImGuiMouseCursor_TextInput, VTK_CURSOR_DEFAULT }, { ImGuiMouseCursor_ResizeAll, VTK_CURSOR_SIZEALL },
    { ImGuiMouseCursor_ResizeNS, VTK_CURSOR_SIZENS }, { ImGuiMouseCursor_ResizeEW, VTK_CURSOR_SIZEWE },
    { ImGuiMouseCursor_ResizeNESW, VTK_CURSOR_SIZENE }, { ImGuiMouseCursor_ResizeNWSE, VTK_CURSOR_SIZENW },
    { ImGuiMouseCursor_Hand, VTK_CURSOR_HAND }, { ImGuiMouseCursor_NotAllowed, VTK_CURSOR_DEFAULT } });
}
...
void vtkDearImGuiInjector::Inject(vtkRenderWindowInteractor* interactor) {
  if (this->FinishedSetup) { vtkErrorMacro(<< "Inject must be called only once!"); }
...
bool vtkDearImGuiInjector::SetUp(vtkRenderWindow* renWin) {
  ...
  if (this->FinishedSetup) { return true; }
...
void vtkDearImGuiInjector::TearDown(vtkObject* caller, unsigned long eid, void* callData) {
  ...
  if (interactor != nullptr) { interactor->SetDone(true); }
...
void vtkDearImGuiInjector::BeginDearImGuiOverlay(vtkObject* caller, unsigned long eid, void* callData) {
  ...
  auto currentTime =
    double(duration_cast<milliseconds>(high_resolution_clock::now().time_since_epoch()).count()) / 1000.;
...
void vtkDearImGuiInjector::InstallEventCallback(vtkRenderWindowInteractor* interactor) {
  ...
  this->CurrentIStyle->AddObserver(vtkCommand::LeftButtonReleaseEvent, this->EventInterceptor, 1.0);

  this->CurrentIStyle->AddObserver(vtkCommand::LeftButtonDoubleClickEvent, this->EventInterceptor, 1.0);

  this->CurrentIStyle->AddObserver(vtkCommand::MiddleButtonPressEvent, this->EventInterceptor, 1.0);

  this->CurrentIStyle->AddObserver(vtkCommand::MiddleButtonReleaseEvent, this->EventInterceptor, 1.0);

  this->CurrentIStyle->AddObserver(vtkCommand::MiddleButtonDoubleClickEvent, this->EventInterceptor, 1.0);

  this->CurrentIStyle->AddObserver(vtkCommand::RightButtonPressEvent, this->EventInterceptor, 1.0);

  this->CurrentIStyle->AddObserver(vtkCommand::RightButtonReleaseEvent, this->EventInterceptor, 1.0);

  this->CurrentIStyle->AddObserver(vtkCommand::RightButtonDoubleClickEvent, this->EventInterceptor, 1.0);

  this->CurrentIStyle->AddObserver(vtkCommand::MouseWheelForwardEvent, this->EventInterceptor, 1.0);

  this->CurrentIStyle->AddObserver(vtkCommand::MouseWheelBackwardEvent, this->EventInterceptor, 1.0);
...
void vtkDearImGuiInjector::UpdateMousePosAndButtons(vtkRenderWindowInteractor* interactor) {
  // Update buttons
  ImGuiIO& io = ImGui::GetIO();
  for (int i = 0; i < 3; i++) { io.MouseDown[i] = this->MouseJustPressed[i]; }
  ...
  if (this->Focused) {
    ...
    io.MousePos = ImVec2(static_cast<float>(mouse_x), io.DisplaySize.y - static_cast<float>(mouse_y));
  }
}
...
void vtkDearImGuiInjector::PumpEvents(vtkObject* caller, unsigned long eid, void* callData) {
  ...
  emscripten_set_main_loop_arg(
    &mainLoopCallback, (void*)this, 0, vtkRenderWindowInteractor::InteractorManagesTheEventLoop);
#else
  while (!interactor->GetDone()) { mainLoopCallback(reinterpret_cast<void*>(this)); }
#endif
}

//------------------------------------------------------------------------------
void vtkDearImGuiInjector::InterceptEvent(vtkObject* caller, unsigned long eid, void* clientData, void* callData) {
  // auto interactor = vtkRenderWindowInteractor::SafeDownCast(caller);
  ...
  switch (eid) {
    ...
    case vtkCommand::MouseMoveEvent: {
      if (!io.WantCaptureMouse || (io.WantCaptureMouse && self->GrabMouse)) { iStyle->OnMouseMove(); }
      break;
    }
    case vtkCommand::LeftButtonPressEvent: {
      self->MouseJustPressed[ImGuiMouseButton_Left] = true;
      if (!io.WantCaptureMouse || (io.WantCaptureMouse && self->GrabMouse)) { iStyle->OnLeftButtonDown(); }
      break;
    }
    case vtkCommand::LeftButtonReleaseEvent: {
      self->MouseJustPressed[ImGuiMouseButton_Left] = false;
      if (!io.WantCaptureMouse || (io.WantCaptureMouse && self->GrabMouse)) { iStyle->OnLeftButtonUp(); }
      break;
    }
    case vtkCommand::LeftButtonDoubleClickEvent: {
      io.MouseDoubleClicked[ImGuiMouseButton_Left] = true;
      if (!io.WantCaptureMouse || (io.WantCaptureMouse && self->GrabMouse)) { iStyle->OnLeftButtonDoubleClick(); }
      break;
    }
    case vtkCommand::MiddleButtonPressEvent: {
      self->MouseJustPressed[ImGuiMouseButton_Middle] = true;
      if (!io.WantCaptureMouse || (io.WantCaptureMouse && self->GrabMouse)) { iStyle->OnMiddleButtonDown(); }
      break;
    }
    case vtkCommand::MiddleButtonReleaseEvent: {
      self->MouseJustPressed[ImGuiMouseButton_Middle] = false;
      if (!io.WantCaptureMouse || (io.WantCaptureMouse && self->GrabMouse)) { iStyle->OnMiddleButtonUp(); }
      break;
    }
    case vtkCommand::MiddleButtonDoubleClickEvent: {
      io.MouseDoubleClicked[ImGuiMouseButton_Middle] = true;
      if (!io.WantCaptureMouse || (io.WantCaptureMouse && self->GrabMouse)) { iStyle->OnMiddleButtonDoubleClick(); }
      break;
    }
    case vtkCommand::RightButtonPressEvent: {
      self->MouseJustPressed[ImGuiMouseButton_Right] = true;
      if (!io.WantCaptureMouse || (io.WantCaptureMouse && self->GrabMouse)) { iStyle->OnRightButtonDown(); }
      break;
    }
    case vtkCommand::RightButtonReleaseEvent: {
      self->MouseJustPressed[ImGuiMouseButton_Right] = false;
      if (!io.WantCaptureMouse || (io.WantCaptureMouse && self->GrabMouse)) { iStyle->OnRightButtonUp(); }
      break;
    }
    case vtkCommand::RightButtonDoubleClickEvent: {
      io.MouseDoubleClicked[ImGuiMouseButton_Right] = true;
      if (!io.WantCaptureMouse || (io.WantCaptureMouse && self->GrabMouse)) { iStyle->OnRightButtonDoubleClick(); }
      break;
    }
    case vtkCommand::MouseWheelBackwardEvent: {
      io.MouseWheel = -1;
      if (!io.WantCaptureMouse || (io.WantCaptureMouse && self->GrabMouse)) { iStyle->OnMouseWheelBackward(); }
      break;
    }
    case vtkCommand::MouseWheelForwardEvent: {
      io.MouseWheel = 1;
      if (!io.WantCaptureMouse || (io.WantCaptureMouse && self->GrabMouse)) { iStyle->OnMouseWheelForward(); }
      break;
    }
    case vtkCommand::MouseWheelLeftEvent: {
      io.MouseWheelH = 1;
      if (!io.WantCaptureMouse || (io.WantCaptureMouse && self->GrabMouse)) { iStyle->OnMouseWheelLeft(); }
      break;
    }
    case vtkCommand::MouseWheelRightEvent: {
      io.MouseWheelH = -1;
      if (!io.WantCaptureMouse || (io.WantCaptureMouse && self->GrabMouse)) { iStyle->OnMouseWheelRight(); }
      break;
    }
...
#else
      char key_code = iStyle->GetInteractor()->GetKeyCode();
      io.AddInputCharacter(key_code);
      // io.AddInputCharactersUTF8(&key_code);
      // if (!keySym.empty() && keySym.length() == 1) { io.AddInputCharactersUTF8(keySym.c_str()); }
      // if (key_code > 0 && key_code < 128) {}

      // std::cout << " key_code = " << key_code << ", keySym.c_str() = " << keySym << "\n";
#endif

      if (!io.WantCaptureKeyboard || (io.WantCaptureKeyboard && self->GrabKeyboard)) { iStyle->OnChar(); }
      break;
    }
    ...
    case vtkCommand::KeyReleaseEvent: {
      ...
      std::cout << " key_code = " << key << ", keySym.c_str() = " << keySym << "\n";
      ...
      io.KeySuper = (keySym == "Win_L") || (keySym == "Win_R") || (keySym == "Super_L") || (keySym == "Super_R");
      // io.KeySuper &= down;
#elif defined(USES_SDL2)
      unsigned int key = SDL_GetScancodeFromKey(static_cast<unsigned int>(iStyle->GetInteractor()->GetKeyCode()));
      if (!key) { key = static_cast<unsigned int>(iStyle->GetInteractor()->GetKeyCode()); }
